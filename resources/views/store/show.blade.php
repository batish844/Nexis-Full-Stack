...
@section('content')
    ...
    <div class="container mx-auto my-6">
        <a href="{{ url()->previous() }}"
            class="inline-flex items-center text-md md:text-lg justify-center gap-2 text-blue-700 py-2 px-4 sm:px-6 rounded-lg transition-all transform hover:scale-105 focus:outline-none focus:ring-4 focus:ring-blue-300">
            ← Back to Store
        </a>
        <div class="flex flex-wrap lg:items-stretch justify-center lg:space-x-16 rounded">
            ...
            <div
                ...
                <div class="mt-6">
                    ...
                    <div class="flex flex-wrap items-center space-x-2 sm:space-x-4">
                        @foreach ($item->Size as $size)
                            ...
                            @else
                                @if ($loop->first)
                                    <label for="shoeSizes" class="text-md font-semibold text-gray-700 mb-2 block">Select
                                        Size:</label>
                                    <select id="shoeSizes" name="size"
                                        class="w-full sm:w-64 px-4 py-2 border border-gray-300 rounded-lg shadow-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 transition">
                ...
                <div class="mt-4">
                    <!-- Notification Area -->
                    <div id="response-message" class="hidden rounded-lg text-center text-sm mx-auto font-semibold mb-4">
                    </div>
        ...
        <div id="reviewSection"
            class="mt-12 p-3 sm:p-6 px-4 md:px-12 mx-4 sm:mx-5 md:mx-10 border-t border-gray-300 bg-gray-100 rounded-lg">
            ...
            <div class="mt-10 bg-white p-4 sm:p-6 rounded-lg shadow-md">
                ...
                @if ($existingReview)
                    <p class="hidden text-sm text-gray-600 mb-4">Review to earn 1 point</p>
                @else
                    <p class="text-sm text-gray-600 mb-4">
                        <i class="fas fa-trophy text-yellow-400 text-xl"></i>
                        Review to earn 1 point
                    </p>
                @endif
        ...
        document.addEventListener('DOMContentLoaded', async function() {
            ...
            sizeLabels.forEach(label => {
                label.addEventListener('click', () => {
                    ...
                    document.querySelectorAll('.size-checkbox').forEach(cb => cb.checked =
                        false);
                    ...
                    sizeButton.classList.remove('bg-gray-200', 'text-gray-800',
                        'border-gray-300');
                ...
                if (remainingStock <= 0) {
                    ...
                    setTimeout(() => {
                        ...
                        setTimeout(() => {
                            ...
                            responseMessageEl.style.opacity =
                                '1'; // Reset opacity for future use
                        }, 500);
                    }, 3000);
                    return;
                } else {
            ...
            addToCartBtn.addEventListener('click', async () => {
                ...
                const shoeSizesDropdown = document.querySelector('#shoeSizes');
                let selectedSize;

                if (sizeRadio) {
                    selectedSize = sizeRadio.value;
                } else if (shoeSizesDropdown && shoeSizesDropdown.value) {
                    selectedSize = shoeSizesDropdown.value;
                } else {
                    selectedSize = null;
                }
                ...
                if (quantityToAdd > remainingStock) {
                    responseMessageEl.textContent =
                        `Only ${remainingStock} items remaining in stock.`;
                    responseMessageEl.classList.remove('hidden', 'text-green-500');
                    responseMessageEl.classList.add('text-red-500');
                    setTimeout(() => {
                        responseMessageEl.style.transition = 'opacity 0.5s ease';
