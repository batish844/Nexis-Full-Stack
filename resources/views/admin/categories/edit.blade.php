...
<div class="mx-auto">
    ...
    <div class="bg-white rounded-xl shadow-lg border  mb-10 p-8">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-y-6 gap-x-10">
            <div>
                <h2 class="text-sm font-semibold text-blue-700 uppercase mb-2">Category Name</h2>
                <p class="text-xl font-bold text-gray-800">{{ $category->Name }}</p>
            </div>
            <div>
                <h2 class="text-sm font-semibold text-blue-700 uppercase mb-2">Description</h2>
            ...
            <div>
                ...
                <p class="text-gray-700">{{ $category->created_at->format('d M Y, h:i A') }}</p>
            </div>
            <div>
                <h2 class="text-sm font-semibold text-blue-700 uppercase mb-2">Last Updated</h2>
                <p class="text-gray-700">{{ $category->updated_at->format('d M Y, h:i A') }}</p>
            </div>
        </div>
    </div>
    <div class="bg-white rounded-xl shadow-lg border  p-8">
        ...
        <form action="{{ route('categories.update', $category->CategoryID) }}" method="POST"
            ...
            <div>
                <label for="description" class="block text-blue-700 font-semibold mb-2">Description</label>
                <input id="description" name="description" placeholder="Enter description" value="{{ $category->Description }}"
