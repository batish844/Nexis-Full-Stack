          ...
          <tr class="border-b hover:bg-gray-50 transition duration-200 cart-item-row"
            ...
            <td class="px-6 py-4 flex items-center">
              ...
              <div class="text-gray-800 font-medium">
                <div class="text-2xl font-semibold text-gray-800 py-2">{{ $cartItem->item->Name }}</div>
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    const updateCart = async (itemID, size, action) => {
      try {
        ...
        if (!response.ok || !data.success) throw new Error(data.message || 'Failed to update cart.');
